...
static MatenuMenuItem ** gtk_menu_shell_get_item_array(GtkMenuShell * menu_shell, gint * array_length) {
	MatenuMenuItem ** rt =
...
static void gtk_menu_shell_set_item_array(GtkMenuShell * menu_shell,
	MatenuMenuItem ** array, gint array_length) {
	MatenuMenuItem ** old_array =
	g_object_get_data(G_OBJECT(menu_shell),
	            "item-array");
	if(old_array != array) {
...
void gtk_menu_shell_remove_all(GtkMenuShell * menu_shell) {
	int array_length = 0;
	int i;
	MatenuMenuItem **array = gtk_menu_shell_get_item_array(menu_shell, &array_length);
...
int gtk_menu_shell_get_length(GtkMenuShell * menu_shell) {
	...
	for(i = array_length - 1; i >= 0; i--) {
		if(matenu_menu_item_get_truncated(array[i])) length --;
	}
	return length;
}
/**
 ...
 * non-matenu-item is not counted.
 * */
void gtk_menu_shell_set_length(GtkMenuShell * menu_shell, gint length) {
	...
	if( array_length < length) {
		MatenuMenuItem ** new_array = g_new0(MatenuMenuItem*, length);
		...
		for(i = array_length; i < length; i++) {
			MatenuMenuItem * item = matenu_menu_item_new();
	...
	for(i = 0; i < array_length; i++) {
		if(i >= length) {
			matenu_menu_item_set_truncated(array[i], TRUE);
		} else {
			matenu_menu_item_set_truncated(array[i], FALSE);
		}
	}
}

GtkMenuItem * gtk_menu_shell_get_item(GtkMenuShell * menu_shell, gint position) {
	MatenuMenuItem ** array = NULL;
	gint length = 0;
